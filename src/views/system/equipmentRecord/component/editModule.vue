          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
            <el-form-item label="状态">
              <el-radio-group v-model="state.ruleForm.status">
                <el-radio v-for="dict in state.statusOptions" :key="dict.dictValue" :label="dict.dictValue">{{
                  dict.dictLabel }}
                </el-radio>
              </el-radio-group>
            </el-form-item>
...
import { sys_user_sex, sys_normal_disable } from "@/utils/dictionary";
...
const openDialog = (row: any) => {
  ...
  if (row && row.userId && row.userId != undefined && row.userId != 0) {
    ...
    getUser(row.userId).then((response: any) => {
      console.log(response,"休息休息了解离开家");
  ...
  state.loading = false;
  // 查询显示性別数据字典
  state.sexOptions = sys_user_sex
  // 查询显示狀態数据字典
  state.statusOptions = sys_normal_disable
};
// 关闭弹窗
